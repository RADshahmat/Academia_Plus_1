...
router.get("/assignmentclass1", async function (req, res) {
  ...
  const gline = await run(
    'SELECT * FROM "ACADEMIA_PLUS_NEW"."ASSIGNMENTS" WHERE CLASS = \'class2\''
  );
  console.log(gline.data);
...
router.post(
  ...
  async function (req, res) {
    ...
    const class_stu = await run(
      `select CLASS from STUDENTS where ID=:student`,
      { student: student_id }
    );
    console.log(data, image_name, class_stu.data[0][0]);
    const det = await run(
      `insert into SUB_ASSIGNMENTS (ID,UPLOADED_FILE,TURNIN,CLASS,ASSIGNMENT_ID) values(
...
router.post("/turnIn", async function (req, res) {
  ...
  const response = await run(
    `UPDATE SUB_ASSIGNMENTS SET TURNIN = :1 WHERE ASSIGNMENT_ID = :2 AND ID = :3`,
    {
      1: det.turnIn,
      2: det.assignmentId,
